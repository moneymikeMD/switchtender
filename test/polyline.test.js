...
test('getJson and getText never throw, name the failure class, add the query at call time and send one User-Agent', async () => {
  ...
  assert.deepEqual(await getJson('https://x.test/a', { fetchImpl: async () => ({ ok: false, status: 503 }) }), { error: 'HTTP 503', status: 503, retryAfter: null });
  ...
  assert.deepEqual(
    await getJson('https://x.test/a', {
      fetchImpl: async () => ({ ok: false, status: 429, headers: new Headers({ 'retry-after': '2' }) }),
    }),
