 ...
 * @returns { options, incidents, closures, maryland, verdict, spoken, logged }
...
export async function runVerdict(config, { fetchImpl = fetch, now = new Date(), log = true } = {}) {
  ...
  const closuresPromise = fetchClosures(config, fetchImpl, { now: now.getTime() });
  ...
  const [incidents, closures, maryland] = await Promise.all([
    incidentsPromise,
    closuresPromise,
    fetchChart(options.driveThrough.points, fetchImpl),
  ]);
  ...
  return { options, incidents, closures, maryland, verdict, spoken: speak(verdict), logged };
}
